...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('5NH18NIMzU9fBiM') ) {
      let user = new User({
        'password':'5NH18NIMzU9fBiM',
        'username':'Annie56',
    ...
    let admin = await User.findOne({
      'username':'Torrey_Rath',
      'isDeleted':false,
      'isActive':true
    });
    if (!admin || !admin.isPasswordMatch('DHZ1KWnjjOykf7g') ) {
      let admin = new User({
        'password':'DHZ1KWnjjOykf7g',
        'username':'Torrey_Rath',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('5NH18NIMzU9fBiM') && userRole){
    ...
    if (admin && admin.isPasswordMatch('DHZ1KWnjjOykf7g') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
